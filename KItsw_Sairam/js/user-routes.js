...
function initializeRouter(promisePool) {
    ...
    // ─── AUTH MIDDLEWARE ──────────────────────────────────────────────────────────
    // Verifies JWT token on every request to this router
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            // Get total count — includes LEFT JOIN so s.full_name is available in WHERE
            const countQuery = `
            ...
            // FIX: usersQuery now has the same LEFT JOIN as countQuery
            // so s.full_name in the WHERE clause resolves correctly
            const usersQuery = `
            ...
            // FIX: Use SQL LIMIT/OFFSET directly instead of slicing in JS
            const offset = (page - 1) * limit;
            const [users] = await promisePool.execute(usersQuery, [...params, limit, offset]);
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const [result] = await promisePool.execute(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [username, email, passwordHash, userRole, staffId || null, true, req.user?.user_id || null]);
