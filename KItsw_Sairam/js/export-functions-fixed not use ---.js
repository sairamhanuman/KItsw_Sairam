...
window.exportToPDF = async function(data) {
    ...
    try {
        // ── FIX: Always fetch notification details fresh from API ──────────
        const urlParams      = new URLSearchParams(window.location.search);
        const notificationId = urlParams.get('notificationId');
        ...
        if (!notificationDetails) {
            console.log('⚠️ window.notificationDetails missing — fetching from API...');
            try {
                const res    = await fetch(`/api/exam-notifications/${notificationId}`);
                const result = await res.json();
                if (result.status === 'success' && result.data) {
                    notificationDetails        = result.data;
                    window.notificationDetails = result.data; // cache it
                    console.log('✅ Fetched notification details:', notificationDetails);
                } else {
                    throw new Error('API returned error: ' + (result.message || 'unknown'));
                }
            } catch (fetchErr) {
                console.error('❌ Failed to fetch notification details:', fetchErr);
                Swal.fire('Error', 'Could not load notification details: ' + fetchErr.message, 'error');
                return;
            }
        }
        // ── END FIX ────────────────────────────────────────────────────────
        ...
        const notifId      = notificationDetails.notification_id || notificationId || '';
        ...
        function calcRowHeight(fullKey) {
            ...
            return Math.max(14, maxContentH + 6);
        }

        const rowHeights = {};
        dateKeys.forEach(key => { rowHeights[key] = calcRowHeight(key); });
        ...
        dateKeys.forEach((fullKey, rowIdx) => {
            ...
            const rowH = rowHeights[fullKey];

            if (rowIdx % 2 === 0) doc.setFillColor(255, 255, 255);
        ...
        if (!batch_id) {
            try {
                ...
                if (notifResult.status === 'success') {
                    // Cache it for export use too
                    window.notificationDetails = notifResult.data;
