...
function sgpaSlab(sgpa, pf) {
  if (pf === 'F') return 'Fail';
  const pct = (sgpa - 0.5) * 10;
  if (pct >= 90) return 'More 90%';
...
function initializeRouter(db) {
  ...
  router.get('/result/analysis', async (req, res) => {
    ...
    try {
      ...
      const result = rows.map(r => {
        ...
        if (!resultMonth && r.exam_date) resultMonth = fmtMonth(r.exam_date);
  ...
  router.get('/notice/data', async (req, res) => {
    ...
    try {
      const [rows] = await db.query(`
        SELECT rd.roll_no, rd.branch, rd.pf_status, rd.exam_date,
      ...
      rows.forEach(r => {
        const { roll_no, branch, pf_status, exam_date, sgpa_division, subject_name, cw_grade } = r;
        if (!resultMonth && exam_date) resultMonth = fmtMonth(exam_date);
        if (!branchMap[branch]) {
