...
router.get('/students', async (req, res) => {
    try {
        ...
        for (const student of students) {
            // Filter timetable entries for this student's branch + semester + regulation
            // semester_id and regulation_id are now stored directly in exam_timetable_entries
            // (stamped from exam_notifications at save time) — no JOIN to subject_master needed
            ...
            for (const entry of studentTimetable) {
                ...
                // Only resolve if there are MULTIPLE subjects for this elective_name+branch in timetable
                // (meaning student needs to pick one). If only 1 subject exists, use it directly.
                if (entry.is_elective === 1 && entry.is_under_group === 1 && entry.elective_name) {
                    // Count how many timetable entries share the same elective_name+branch+date
                    const siblingsOnSameDate = timetableEntries.filter(
                    ...
                    if (siblingsOnSameDate.length > 1) {
                        // Multiple elective options → check which one this student chose
                        ...
                        if (electiveMapping.length === 0) {
                            // Student has no elective mapping → skip this entry
                            // (another sibling entry will also be checked, avoid duplicates)
                            continue;
                        }

                        // Only include the entry that matches student's chosen subject
                        if (electiveMapping[0].subject_id !== entry.subject_id) {
                            continue; // This is not the student's chosen elective
                        }
                    ...
                    // else: only 1 subject for this elective → use timetable subject directly (no mapping needed)
                }
