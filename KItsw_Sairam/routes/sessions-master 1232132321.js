...
router.post('/', async (req, res) => {
    try {
        ...
        // Validate session_group value
        if (!['FN', 'AN'].includes(session_group)) {
            return res.status(400).json({
                status: 'error',
                message: 'Invalid session_group. Must be FN or AN'
            });
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // Validate session_group value if provided
        ...
        if (session_group !== undefined) {
            updateFields.push('session_group = ?');
            updateValues.push(session_group);
        }
