module.exports = (promisePool) => {
        ...
        try {
            ...
            await promisePool.query(`
                INSERT INTO subject_faculty_allotment
                    (programme_id, branch_id, semester_id, regulation_id,
                     subject_id, allotment_type, batch_id, section_id, staff_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `, [
                programme_id, branch_id, semester_id, regulation_id,
                subject_id, allotment_type || 'Theory', batch_id, section_id, staff_id
            ]);
            ...
            if (error.code === 'ER_DUP_ENTRY') {
                return res.status(409).json({
                    success: false,
                    message: 'This subject is already allotted to this faculty for the selected section.'
                });
            }
            res.status(500).json({ success: false, message: error.message });
        }
    ...
    router.delete('/:id', async (req, res) => {
        try {
            await promisePool.query(
                `UPDATE subject_faculty_allotment SET deleted_at = NOW() WHERE allotment_id = ?`,
                [req.params.id]
            );
