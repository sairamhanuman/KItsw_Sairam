...
BEGIN
    ...
    -- Get programme code (handle case where not found)
    SELECT programme_code INTO programme_code 
    FROM programme_master 
    WHERE programme_id = NEW.programme_id AND is_active = 1
    LIMIT 1;
    ...
    -- Get month name (handle case where not found)
    SELECT month_name INTO month_name 
    FROM month_year_master 
    WHERE month_year_id = NEW.month_year_id AND is_active = 1
    LIMIT 1;
